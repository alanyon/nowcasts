...
def main():
    ...
    for vdt in vdts:
        ...
        fname = (f'{DATADIR}/'
                 f'verification/88_{vdt.strftime("%Y%m%d%H%M")}_scores.csv')
    ...
    fig.legend(handles, labels, loc='upper right', title='Scale', fontsize=18,
               title_fontproperties={'weight': 'bold', 'size': 20})

    # Save and close plot
    fname = f'{DATADIR}/plots/verification_{START}_{END}_subplots.png'
    fig.savefig(fname)
    plt.close()
    ...
    # Save and close plot
    fname = f'{DATADIR}/plots/verification_{START}_{END}_all_plot.png'
    fig.savefig(fname)
    plt.close()
    ...
    # Save and close plot
    fname = f'{DATADIR}/plots/verification_{START}_{END}__heatmap.png'
    fig.savefig(fname)
    plt.close()
